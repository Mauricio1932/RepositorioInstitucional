...
function IndeceContenido({column}) {

    console.log("dta ",column)
    ...
    //     { tittle:'Contenido',nombre: contenido }, 
            ...
            <div className="my-3 p-3 bg-body rounded shadow-sm">
                <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                    {column.map((content, index) => (
                        <Columna key={index} titleColumn={content.tittle} contenido={content.nombre}/>
                    ))}
                </div>
            </div>
        </>
